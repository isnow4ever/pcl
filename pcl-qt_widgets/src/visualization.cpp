...
bool
Visualization::estimateTfBetweenPlanes(const ModelCoefficients::Ptr coeff_model,
	const ModelCoefficients::Ptr coeff_data,
	Eigen::Matrix4d &datum_transform)
{
	Eigen::Vector4d dm_centroid, dd_centroid;
	pcl::compute3DCentroid(*datum_model, dm_centroid);
	pcl::compute3DCentroid(*datum_data, dd_centroid);
	double aa = dm_centroid[0] - dd_centroid[0];
	double bb = dm_centroid[1] - dd_centroid[1];
	double cc = dm_centroid[2] - dd_centroid[2];

	Eigen::Vector3d na(coeff_data->values[0], coeff_data->values[1], coeff_data->values[2]);
	Eigen::Vector3d nb(coeff_model->values[0], coeff_model->values[1], coeff_model->values[2]);
	na.normalize();
	nb.normalize();
	Eigen::Vector3d nn = na.cross(nb);
	//float half_cos = sqrt(0.5f * (1.f + na.dot(nb)));
	//float half_sin = sqrt(0.5f * (1.f - na.dot(nb)));

	Eigen::AngleAxisd angle_axis(acos(na.dot(nb)), na.cross(nb));

	Eigen::Quaterniond qq(angle_axis);
	Eigen::Matrix3d rotationM = qq.matrix();
	Eigen::Affine3d ro(rotationM);
	Eigen::Affine3d tl(Eigen::Translation3d(aa, bb, cc));
